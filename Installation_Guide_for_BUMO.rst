...
Installing by Compilation means that the source code of the BUMO node is first compiled into machine code that can be recognized by the computer and then installed. Installing by Compilation consists of three parts: `Installing Dependencies`_, `Compiling the BUMO Source Code`_, `Installing the BUMO Node`_.
...
5. Enter the following command to complete the compilation of the BUMO source code. The message below shows that the compilation is successful.
...
2. Enter the following command to complete the installation. The message below shows that the installation is successful.
...
Installing with a package refers to installing the BUMO node with an installation package. Installing the BUMO node with the installation package consists of five parts: `Obtaining the Installation Package and Extracting It `_, `Registering the Services`_, `Modifying the Service Startup Directory`_, `Setting the Boot Start `_, `Selecting the Configuration File for the Running Environment`_ .
...
2. Enter the following command to register the service of bumod.
...
The ``validation_address`` and ``validation_private_key`` from the ledger of verification node must match. And you must fill ``validation_address`` of all the verification nodes into ``genesis.validators``.
...
2. Enter the following command to clear the database.
