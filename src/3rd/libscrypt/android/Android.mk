
LOCAL_PATH:= $(call my-dir)
TOP_PATH:= $(LOCAL_PATH)/../
SRC_PATH:= $(TOP_PATH)/

include $(CLEAR_VARS)

LOCAL_ARM_MODE := arm
LOCAL_PRELINK_MODULE := false
LOCAL_MODULE_TAGS := optional

LOCAL_SRC_SCRYPT_FILES := \
 			$(SRC_PATH)/crypto_scrypt-nosse.c\
 			$(SRC_PATH)/crypto-mcf.c\
			$(SRC_PATH)/b64.c\
			$(SRC_PATH)/sha256.c\
			$(SRC_PATH)/crypto-scrypt-saltgen.c\
			$(SRC_PATH)/crypto_scrypt-check.c\
			$(SRC_PATH)/crypto_scrypt-hash.c\
 			$(SRC_PATH)/slowequals.c

LOCAL_SRC_FILES := \
	$(LOCAL_SRC_SCRYPT_FILES) 
	

LOCAL_C_INCLUDES += \
	$(SRC_PATH)/ \
	$(TOP_PATH)/include

#LOCAL_LDLIBS += -L$(SYSROOT)/usr/lib -llog 

LOCAL_CPPFLAGS := -DHAMMER_TIME=1 \
		  -DHASHNAMESPACE=__gnu_cxx \
		  -DHASH_NAMESPACE=__gnu_cxx \
		  -DDISABLE_DYNAMIC_CAST \
		  -D_REENTRANT \
		  -DANDROID \
		  -D_FORTIFY_SOURCE=2

LOCAL_CFLAGS := -fexpensive-optimizations -fexceptions -pthread -DHAVE_NEON=1 \
		-mfpu=neon -mfloat-abi=softfp -flax-vector-conversions -fPIC -D__STDC_CONSTANT_MACROS -Wno-sign-compare -Wno-switch 

LOCAL_MODULE:= libscrypt

include $(BUILD_STATIC_LIBRARY)
#include $(BUILD_SHARED_LIBRARY)

