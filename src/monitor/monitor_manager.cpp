...
namespace bumo {
	...
	bumo::Connection * MonitorManager::CreateConnectObject(bumo::server *server_h, bumo::client *client_, bumo::tls_server *tls_server_h, 
		bumo::tls_client *tls_client_h, bumo::connection_hdl con, const std::string &uri, int64_t id) {
	...
	bool MonitorManager::OnMonitorHello(protocol::WsMessage &message, int64_t conn_id) {
		...
		do {
			...
			reg.set_blockchain_version(bumo::General::BUMO_VERSION);
