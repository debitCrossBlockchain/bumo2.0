...
在Linux系统中支持两种安装方式：[编译安装](/bian-yi-an-zhuang.md)和[安装包安装](/an-zhuang-bao-an-zhuang.md)。
...
编译安装是指先将BUMO节点的源代码编译成计算机能识别的机器码然后再进行安装。编译安装由三部分构成：[安装依赖](/an-zhuang-yi-lai.md)，[编译BUMO源代码](/bian-yi-an-zhuang/bian-yi.md)，[安装BUMO节点](/bian-yi-an-zhuang/an-zhuang.md)。
