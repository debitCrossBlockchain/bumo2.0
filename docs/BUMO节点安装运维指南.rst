...
.. note:: 至此就完成了BUMO节点的安装。在启动bumo服务之前还需要 选择运行环境的配置文件_ 。
...
安装包安装是指以安装包的方式来安装BUMO节点。以安装包的方式来安装BUMO节点包括两个步骤：`获取安装包并解压 <MacOS中获取安装包并解压>`_ 、`选择运行环境的配置文件 <MacOS中选择运行环境的配置文件>`_。
