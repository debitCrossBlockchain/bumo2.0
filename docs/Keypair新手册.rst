...
.. attention:: 关于Prefix、Version以及Checksum请查看表1。
...
.. attention:: 关于Prefix、Version以及Checksum请查看表2。
...
.. attention:: 关于Prefix、Version以及Checksum请查看表3。
...
.. attention:: nonce值需要在第1步中获取值的基础上加1。
...
通过 Keypair 中的
生成地址__ \ 生成的新账户B的地址是\ ``buQoP2eRymAcUm3uvWgQ8RnjtrSnXBXfAzsV``\ ，填充的json数据如下：
...
.. attention:: 这里的nonce值不是6，没有连续，因此该交易会超时，不会成功。
