...
Calling the interface to generate a transition_blob includes the following steps:
...
3.By calling the getTransactionBlob interface, the json data generated in Step 2 is passed as a parameter, and a transaction hash and transaction_blob are obtained to implement transaction serialization. The format is shown below:
...
```
    message OperationPayAsset {
        ...
        Asset asset = 2;                         // Asset
        string input = 3;                        // Input paramter for contracts
    }
```
...
This data strucutre is for identifying the uniqueness of asset.
...
```
>**Attention**：The nounce value is not 6, so this transaction would fail.
