...
clean:
	if [ -d ./lib ]; then ls | grep -v icudtl.dat | grep -v natives_blob.bin | grep -v snapshot_blob.bin | xargs rm -rf bin/* && cd lib/ && ls | grep -v http |grep -v ed25519 | xargs rm -rf; fi
